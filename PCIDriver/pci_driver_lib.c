...
WDC_DEVICE_HANDLE         hDev_0= NULL;
WDC_DEVICE_HANDLE         hDev_1= NULL;
...
WD_DMA * pDmaGroup_0 [10];
//hu//int pDmaGroup_0_flag;
WD_DMA * pDmaGroup_1 [10];
...
DWORD reset_PCI(int deviceX)
{
	...
	TraceLog("hu: Start to run the function: reset_PCI...\n");
	...
	if (NULL == hDev)
	{
		WDC_Err("hu: Failed to find the handle to the PCI driver%d: The PCI driver%d is off.\n",deviceX);
		return 1;   // hu PCI设备未启动
	}
	...
	deviceInfo.pciSlot = scanResult.deviceSlot[deviceX];
	WDC_PciGetDeviceInfo(&deviceInfo);
	hDev = PCI_DRIVER_DeviceOpen(&deviceInfo);
	if (NULL == hDev)
	{
	...
	TraceLog("hu: Succeed to reset the PCI driver%d.\n",deviceX);
	return 0;
...
// 返回值：0 -- 成功 
//         1 -- PCI设备写入失败
DWORD Write(int deviceX,int barX, DWORD dwoffset, UINT32 write32)
{	
	DWORD dwStatus;
	...
	dwStatus = WDC_WriteAddr32(hDev, barX, dwoffset, write32);
	if (WD_STATUS_SUCCESS != dwStatus)
	{
...
{	
	...
	for(i=0; i<256; i++)
	{
		dwStatus = WDC_WriteAddr32(hDev, barX, dwoffset+i*4, *(write256Words+i));
		if (WD_STATUS_SUCCESS != dwStatus)
		{
...
DWORD Read(int deviceX,int barX, DWORD dwoffset, UINT32 *read32)
{
	...
	dwStatus = WDC_ReadAddr32(hDev, barX, dwoffset, read32);
	if (WD_STATUS_SUCCESS != dwStatus)
	{
...
{	
	...
	for(i=0; i<256; i++)
	{
		dwStatus = WDC_ReadAddr32(hDev, barX, dwoffset+i*4, read256Words+i);
		if (WD_STATUS_SUCCESS != dwStatus)
		{
...
DWORD Interrupt_1(int deviceX)
{
	...
	case 1:
		hDev = hDev_1;
		dwStatus = PCI_DRIVER_IntEnable(hDev, DiagIntHandler_1);
		break;
	}
	if (WD_STATUS_SUCCESS != dwStatus)
	{
...
//	Int_Flag = 0;
	...
	//	WDC_DMABufUnlock(pDmaReceive);
		...
		PostMessage(ghWnd_0,WM_CALLBACKMESSAGE_0,Int_Flag,0);
...
//	Int_Flag = 0;
	...
	//	WDC_DMABufUnlock(pDmaReceive);
		...
		PostMessage(ghWnd_1,WM_CALLBACKMESSAGE_1,Int_Flag,0);
...
DWORD DisInterrupt_1(int deviceX)
{
	...
	IntStatus=WDC_IntIsEnabled(hDev);// hu 检查中断是否使能	
	if (IntStatus)
	{
...
DWORD SendInterrupt(int deviceX)
{
	...
	WDC_ReadAddr32(hDev, BAR2, 0x02C00010-0x02800000, &RdReg);
	RdReg = RdReg | 0x80000000;
	WDC_WriteAddr32(hDev, BAR2, 0x02C00010-0x02800000, RdReg);
...
DWORD  ApplyForPhysicalAddress(int deviceX, DWORD dwDMABufSize, PVOID * pBufAddress, DWORD * pBufPhysicalAddress, DWORD * DMAHandle)
{
	...
	case 0:
		...
		{
			dwStatus = WDC_DMAContigBufLock(hDev, pBufAddress, DMA_TO_DEVICE, dwDMABufSize, &pDmaGroup_0[i]);
			if (WD_STATUS_SUCCESS != dwStatus)
			{
				WDC_Err("hu: Failed to apply the physical address of the PCI driver%d. Error 0x%lx - %s\n",deviceX,dwStatus, Stat2Str(dwStatus));
				return 2;
			}
			* pBufPhysicalAddress = (DWORD) pDmaGroup_0[i]->Page[0].pPhysicalAddr;
			* DMAHandle = i;

			dwStatus = WDC_DMASyncCpu(pDmaGroup_0[i]);
			if (WD_STATUS_SUCCESS != dwStatus)
			{
	...
	case 1:
		...
		{
			dwStatus = WDC_DMAContigBufLock(hDev, pBufAddress, DMA_TO_DEVICE, dwDMABufSize, &pDmaGroup_1[i]);	
			if (WD_STATUS_SUCCESS != dwStatus)
			{
				WDC_Err("hu: Failed to apply the physical address of the PCI driver%d. Error 0x%lx - %s\n",deviceX,dwStatus, Stat2Str(dwStatus));
				return 2;
			}
			* pBufPhysicalAddress = (DWORD) pDmaGroup_1[i]->Page[0].pPhysicalAddr;
			* DMAHandle = i;
...
DWORD  FreePhysicalAddress(int deviceX, DWORD DMAHandle)
{
	...
	case 0:
	    ...
	    if(pDmaGroup_0[i] != 0)
		{
			dwStatus = WDC_DMASyncIo(pDmaGroup_0[i]);
			if (WD_STATUS_SUCCESS != dwStatus)
			{
		    ...
		    dwStatus = WDC_DMABufUnlock(pDmaGroup_0[i]);
			if (WD_STATUS_SUCCESS != dwStatus)
			{
	...
	case 1:
		...
		if(pDmaGroup_1[i] != 0)
		{
		    ...
		    dwStatus = WDC_DMABufUnlock(pDmaGroup_1[i]);
			if (WD_STATUS_SUCCESS != dwStatus)
			{
				WDC_Err("hu: Failed to free the physical address of the PCI driver%d. Error 0x%lx - %s\n",deviceX,dwStatus, Stat2Str(dwStatus));
				return 2;
...
DWORD Close_PCI(int deviceX)
{
	...
	case 0:
		...
		{
			ii = 1;
			WDC_Err("hu: Failed to close the PCI driver0.\n");
		}
		break;
	case 1:
		hDev = hDev_1;
		...
		{
			ii = 1;
			WDC_Err("hu: Failed to close the PCI driver1.\n");
		}
		break;
	}
